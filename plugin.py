        ...
        if RS485.MyMode == "minimalmodbus":
            if self.functioncode == 3 or self.functioncode == 4:
                try:
                    ...
                    Domoticz.Debug("Modbus connection failure - skipping update")
                    return
   ...
   def UpdateRegister(self,RS485,command,level):
        ...
        if RS485.MyMode == "minimalmodbus":
            try:
                ...
                Domoticz.Debug("Modbus write failed - skipping")
                return
        elif RS485.MyMode == "pymodbus":
            try:
                ...
                Domoticz.Debug("Connection failure: "+str(e))
                Domoticz.Debug("pymodbus write failed - skipping")
                return
        ...
        if RS485.MyMode == "minimalmodbus":
            if self.functioncode == 3 or self.functioncode == 4:
                try:
                    ...
                    Domoticz.Debug("Connection failure: "+str(e))
                    Domoticz.Debug("minimalmodbus connection failure - skipping update")
                    return
        elif RS485.MyMode == "pymodbus":
            if self.functioncode == 3:
                try:
...
class BasePlugin:
    ...
    def onCommand(self, u, Command, Level, Hue):
        ...
        try:
            ...
            sw = self.settings[u-1-50]
            sw.UpdateRegister(self.RS485,Command,Level)
            if Command == "Set Level":
                if sw.Switchtyp == 7:  # Dimmer
                    Devices[u].Update(nValue=2 if Level > 0 else 0, sValue=str(Level))
                elif sw.Type == 242:  # Setpoint
                    Devices[u].Update(nValue=0, sValue=str(Level))
                else:  # Selector
                    Devices[u].Update(nValue=1 if Level > 0 else 0, sValue=str(Level))
            elif Command == "On":
                if sw.Switchtyp == 7:  # Dimmer: On = 100%
                    Devices[u].Update(nValue=2, sValue="100")
                else:
                    Devices[u].Update(nValue=1, sValue="On")
            elif Command == "Off":
                if sw.Switchtyp == 7:  # Dimmer: Off = 0
                    Devices[u].Update(nValue=0, sValue="0")
                else:
